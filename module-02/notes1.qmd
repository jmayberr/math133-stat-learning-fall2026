...
The natural question is: why not just include both predictors in our model if they are significant? In other words, why not look at a model of the form $$Y = f(X_1, X_2) +\varepsilon$$ where $X_1=$ *non.white* and $X_2=$ median.income?

Multiple regression models do just that. They are the natural extension of simple linear models where we are allowed to include more than one predictor $X$ in our systematic information, but still assume it is a linear function of our parameters. In particular, if we have $p$ predictors $X_1,...X_p$, then a multiple regression model is one of the form $$Y = \beta_0  + \beta_1 X_1 + \cdots \beta_p X_p + \varepsilon$$ where as before $\varepsilon$ is mean zero normal random variable with standard deviation $\sigma$.

Our main goal today will be to see how to fit and interpret multiple regression models in R. But before we get to that, a note on notation. Multiple Regression models are often written in the simplified matrix-vector form $$Y = X \beta + \varepsilon$$ where $$X = \begin{bmatrix} 1 & X_1 & \cdots & X_p\end{bmatrix}$$ is what we call the *model matrix* and $$\beta = \begin{bmatrix} \beta_0 \\ \beta_1 \\ \vdots \\ \beta_p \end{bmatrix}$$ is the *coefficient vector*. Notice that under this representation, $\varepsilon$ is what we call a multivariate normal random variable, but if we continue to assume that the errors are independent, you can just think of it as a vector of $n$ independent normal random variables with the same standard deviation. The matrix-vector formulation leads to a nifty trick for estimating $\beta$ and other deeper connections to linear transformations[^1] but in these notes, we'll mainly just use the matrix-vector formulation as a notational tool.

[^1]: See the Module 2 Theory Supplement for a glimpse although a course in linear algebra is required for the full effect.
...
$$sales = \beta_0 + \beta_1 \times TV + \beta_2 \times radio + \beta_3 \times newspaper + \varepsilon$$ The `lm` function is easily modified to accomplish this task:
...
cat("R2 full: ", get_r2(y,yhat_full),
  "R2 reduced: ", get_r2(y,yhat_red))
```
...
Notice that TV goes from 0 to around 300 while radio goes from 0 to around 50. We'll use these as the endpoints of our grid[^2]

[^2]: We only need two points in each direction here because the function is linear, but with nonlinear functions you would need to define a sequence of equally spaced values in teh x and y directions (eg. `x=seq(0,300, by=10)`).
...
```{r}
yhat=predict(sales_lm_reduced,xy_grid)
z_values=matrix(yhat,
         nrow=length(x_values),ncol=length(y_values))
print(z_values)
```

Finally, we plot the predicted values on the z-axis alongside our grid. Note the "t" stands for transpose and flips the yhat matrix so that it matches up with the x- and y-values correctly[^3]:

[^3]: You can ignore the last `layout` command if you dont' care about changing the axes labels. Also [look here](https://plotly.com/r/builtin-colorscales/) for a list of named colorscales that you can use.  
...
An *added variable* or *partial regression* plot shows the relationship between the outcome and each predictors individually after removing the influence of the other predictors from both[^4]. Added variable plots are implemented in the `car` package[^5]:

[^4]: More precisely, to get the added variable plot for $X_j$, we regress $X_j$ on the other predictors to get predicted values $\hat X_j$ and calculate the errors $X_j - \hat X_j$ from our model and do the same for $Y$. Then we fit an SLM to the errors $Y-\hat Y$ based on $X_j - \hat X_j$

[^5]: Here `car` is actually an acronym for "Classification and Regression". Its a package that contains additional diagnostic tools for model fitting which we'll see again in Section 2.3 and other random places throughout the course.

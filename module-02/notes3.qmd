...
*Residuals* are defined as the individual errors $$e_i = y_i- \hat{y}_i$$ They represent "what's left over" after accounting for the trend you think you have identified. Most techniques for identifying issues with SLM or other regression models (a broad topic referred to as regression diagnostics) involve looking at the distribution of residuals to identify problems.
...
Notice how there is a pattern to where we over and under estimate $y$: when $x$ is small or large, we underestimate and when $x$ is in the middle, we overestimate. If calculate and plot the residuals vs fitted values, we will see this pattern show up as a giant "U":
...
[^1]: Specifically, if $$\log \hat y = a + b \log x$$ then $$\hat y = \exp(a + b \log x) = e^a x^b $$ is actually a power function and the coefficient $b$ from the regression is its exponent
...
An additional type of transformation that helps with exponential data[^2] is to take log of only the $y$ values (called a semi-log transformation). As an example, consider the Firestone Propagator dataset which has information on the height of foam (mm) as a function of time since pour (s):

[^2]: Although one can also use nonlinear regression to fit exponential models to data with better outcomes, but that goes beyond the scope of these notes.
...
Fitting a linear model $$\log Height = \beta_0 + \beta_1 Time + \varepsilon$$ gives the following coefficients with a near perfect $R^2$:
...
firestone |>
  ggplot(aes(x=Time,y=Height))+
  geom_point()+
  geom_line(aes(y = exp(predict(fit_lm_semi))), 
            color = "#e35205")
...
At the end of Section 2.1, we noted that added variable plots mark "unusual observations", Dealing with these is one of the biggest grey areas in statistics. This is made trickier by the fact that there are typically two different types of unusual values one should look at:

- outliers, which are points with unusually high residuals, and
- high-leverage points, which are those with unusual $x$-values.

I think of outliers like the punks (they just refuse to follow the trend) and high leverage points as the metal kids (they just stand out in the crowd). Both outliers and high-leverage points can have dramatic effects on your model accuracy although an outlier might not affect your fitted line much if it occurs near $\bar x$, while a high leverage point can dramatically pull the line toward itself even if it isn't technically an outlier.
...
Standardized residuals are the regular residuals converted to z-scores. Assuming the residuals look roughly unbiased from our residual plots, they should have mean 0 so we can covert to z-scores if we divide by their standard deviation. The common method for estimated the standard deviation of residuals is called the Residual Standard Error (RSE): $$RSE = \sqrt{SSE/(n-2)}$$ It's basically the RMSE, but we divide by $n-2$ because of the "degrees of freedom[^3]"

[^3]: Ugh, too many asides. I envy authors of fantasy epics who will just throw out mystery terms and not explain them until four books and 5000 pages later. Basically, degrees of freedom is the sample size minus the number of parameters estimated from the data. For SLM, that's two (slope and intercept).
...
The following graphs show them in the added-variable plots by pre-specifying the points to mark as outliers^[Although note that there is no guarantee that points with high overall standardized residual will stand out in these projections.]. The `cex` command changes the size of the labels. 

...
To fit a logistic model to shot probabilities on the water polo data, we use the glm function in R with the family = "binomial" option. The response variable needs to be recoded as 0/1 or F/T first[^2]

[^2]: That's just the way `glm` works with binary variables. It will take other qualitative variables as predictors, but not a response.
...
```{r}
plot(-5:5,
     (exp(coef(goal_glm)[3]*(-5:5))-1)*100,
     xlab="Change in yj",ylab="% change in odds")
```
...
$$P(Goal|(x,y)) > 0.5$$
The "triangular" boundary for this region[^3] is shown with the shot data in the plot below:

[^3]: Usually the boundary is linear but using absolute value turned it into a triangle
...
```{r}
  ...
  geom_point(size=3)+
  scale_color_manual(name="Goal",values=c("#0072B2","#e35205"))
```
...
For those of you who did the last practice problems, this should be close to the best knn model error rates you got.

## Stratified Sampling

When the classes of our outcome are imbalanced (meaning far fewer of one possible class than the other), reflecting this imbalance in our sample can be desirable. Many machine learning packages use what's called "stratified sampling" to choose training-testing splits. This process selects proportionate samples from all possible classes of the outcome to form the split.

We can perform stratified sampling on the water polo dataset, for example, by modifying our typical algorithm as follows:

```{r}
n0 = sum(shots$Goal_1 == 0)
n1 = sum(shots$Goal_1 == 1)
s0 = sample(which(shots$Goal_1 == 0), 0.7 * n0)
s1= sample(which(shots$Goal_1 == 1), 0.7 * n1)

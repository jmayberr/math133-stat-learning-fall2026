...
In this scenario, the variable we would like to predict is the binary variable Goal. Ultimately, we would like to build a model for predicting Goal based on all the other variables but we are going to start with just the variables xj and yj which contain the "jittered" positions of the shots. The coordinates are relative to the center of the goal with the positive y-axis pointing into the pool and the positive x-axis pointed right; see image below

![Orientation of coordinate system](WP_shot_diagram.png){width=75%}
...
Before building a formal model, let's first visualize the data to get a sense of whether classification is even feasible. Plotting shots based on position and coloring the points by goal/not goal, we can see there will be no perfect way to split the space into decision regions, but there are some clustering of orange and black so maybe we have some hope of beating the "always guess miss model". 
...
To answer the first question, we need to define a metric for assessing how well our model fits the data (akin to RMSE or $R^2$ for regression problems). The most obvious choice is _accuracy_, the proportion of correct predictions: 
$$
accuracy = \frac{1}{n} \sum_{i=1}^n (\hat{y}_i ==y_i) 
$$
The model with the highest accuracy wins. Sometimes (to keep things consistent with regression where we looked to minimize the MSE) instead of maximizing the accuracy, people talk in terms of minimizing the error rate:
$$
ER = 1 - accuracy
$$ 
We'll discuss some issues with accuracy and error rate in 3.3 and Module 6, but use it for now.
...
Pretty close to the true expected error rate of 0.1^[Note: you may get a different error rate if your seed is different.]! Also note that since B is the most dominant class:

```{r}
table(test$class)
```

the null error rate would be 

```{r}
sum(test$class=="A")/nrow(test)
```

We definitely beat that!

Here is a more detailed plot of what we would predict if we use our original dataframe as the training set, but include a finer grid of points for the test set (this is called a plot of the decision region):
...
## Comment on Variable Scaling

In the examples from this section, both predictors $x$ and $y$ are on the same scale, so Euclidean distance treats them comparably and does not unduly favor one variable over the other. In general, however, k-nearest neighbors is sensitive to the scale of the predictors: when variables are measured in different units or have very different ranges, those with larger scales can dominate the distance calculation. Intuitively, KNN “trusts” distance, so whichever variable spreads out more will disproportionately influence who counts as a “neighbor.” 

For example, suppose we multiply the $x$ variable in our data by 1000. Here is how the error rates compare
...
knn_reg=knn(train[,c("x","y")],
             test[,c("x","y")],
             cl=train$class,k=5)

knn_big = knn(train[, c("big_x","y")],
               test[, c("big_x","y")],
               cl = train$class,
               k = 5)

knn_reg_er = sum(knn_reg!=test$class)/n
knn_big_er = sum(knn_big!=test$class)/n

cat("Regular ER = ",knn_reg_er,"; Big  ER=",knn_big_er)

```

The one with big $x$ is twice as big! In such cases, centering and scaling the predictors (for example, using z-scores) before applying KNN can lead to substantial improvements in performance. To illustrate, the error rate after scaling in the big $x$ data is much better: 

```{r}
train_scaled = scale(train[, c("big_x","y")])
test_scaled  = scale(test[, c("big_x","y")],
                       center = attr(train_scaled, "scaled:center"),
                       scale  = attr(train_scaled, "scaled:scale"))
knn_big_scale = knn(train_scaled[, c("big_x","y")],
               test_scaled[, c("big_x","y")],
               cl = train$class,
               k = 5)
knn_scale_er = sum(knn_big_scale!=test$class)/n
cat("Scaled ER:",knn_scale_er)
```

Although we will not explore this issue further here, it will reappear in Module 4 when we discuss regularization methods, where scaling plays a similarly important role.

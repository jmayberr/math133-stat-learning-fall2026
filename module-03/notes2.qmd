...
We will see an easier way of doing stratified sampling when we discuss `tidymodels` in later modules. For now, my advice would be to definitely use it when there is more than an 80-20 imbalance and consider using it if it is more than 60-40 and the dataset is small. The water polo dataset is large and the imbalance is about 70-30 so it probably isn't necessary. For example, in our previous test_set where we didn't stratified, we ended up with a reasonable fraction of goals anyways:    
...
1. Create a 70-30 training-testing split for the data using stratified sampling. 
...
3. Which variables were significant predictors of CHD in your model from 2? 

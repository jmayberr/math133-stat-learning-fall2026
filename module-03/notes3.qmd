---
title: "Module 3.3: Evaluating Classification Models"
---

## Introduction

In these notes we'll take a more detailed look at the framingham model from the practice problem at the end of the last notes. Please take some time to investigate it on your own first before ruining the surprise.
...
We'll start off with a few additional words about interpreting coefficients and then proceed to enrich our understanding of "accuracy" by examining false positive vs false negatives, working towards a better metric of balanced accuracy and a more nuanced picture of how well our models fit classification data.

## Interpreting Coefficients

Recall that if we have a single predictor $X$, the basic model behind logistic regression is of the form $$p(X) = h(\beta_0 + \beta_1 X)$$ where $h$ is the logistic function. Similarly, with multiple predictors we have $$p(X) = h(\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p).$$ While we can use the sign of the $\beta_i$ to determine if the probability of success goes up or down as we increase $X_i$, it is difficult to say anything more explicit about the coefficients from this formulation. First, we have to take a slightly different view of the logistic model and talk about odds.

If you take a walk through the garden of algebra[^1], you can rewrite the logistic equation as $$g(p(X)) =  \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p$$ where $g$ is given by the "link function"[^2] 
$$g(x) = \log(x/(1-x))$$ 
In other words, 
$$\log \left(\frac{p(X)}{1 - p(X)}\right) =  \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p$$ 
The lefthand side is called the "log-odds" function. When we look at a coefficient $\beta_i$, it is telling us the change in the log-odds of success when we increase predictor $X_i$ by 1. For example, in the framingham model, the log-odds of getting CHD are about 0.55 higher for males than females.

[^1]: Credit Parker "Birdman" Spurlin for this phrase.

[^2]: In GLM theory, the link function relates the quantity we are predicting to a linear combination of predictors.

Unfortunately, most of us don't think too well on log scales. Exponentiating the coefficients gives us quantities that are easier to interpret since they are in terms of the odds ratios themselves[^3]. For example, we can ask: how much greater are the odds of a male developing CHD compared to a female? For this, we use the coefficient of "male1" from our model above and exponentiate the result

[^3]: If you aren't too comfortable with odds, consider the following examples. IF a coin is fair, the probability of getting heads is 1/2 but the odds of getting heads is 1 (or 1:1) because we are as likely to get heads as not heads. If the coin is biased with a 2/3 chance of coming up heads, the odds of heads would be 2 (or 2:1) because we are twice as likely to get heads as not heads.
...
We can also ask about the impact of reducing cigarette consumption. Suppose for example we want to know what happens if we reduce cigarette consumption by 20 per day. This leads to the following change in odds of developing CHD:
...
```{r}
tidy_model[-1,] |>  #Note: -1 gets rid of the intercept
...
So far we have been using accuracy as our metric for classification performance, but accuracy alone can be misleading, especially when the classes are imbalanced. To see why, let's start with a motivating example.
...
- A **93.8% chance** of correctly detecting antibodies in someone who has had COVID (this is called *recall* or *sensitivity* or *true positive* rate[^4])
...
[^4]: Its annoying that there are so many names for these terms but that just tells you the concepts are important! They have appeared across many different applications leading to the independent development of different terminologies.

We can organize the results of testing everyone in town into a **confusion matrix** where the rows tell the truth and the columns tell us the predictions[^5]

[^5]: Although be careful of always assuming this orientation. You'll see examples where truth and predictions are switched!
...
For a fitted model, the predictions $\hat y$ take the place of the test results in the example above while the values of the outcome $y$ are the truth to which we compare. It is more common to display the confusion matrix for test data so we'll start by refitting our model using a 70-30 split. Notice that the class imbalance is rather large since only

```{r}
sum(framingham$TenYearCHD==1)/nrow(framingham)
```

of all patients had CHD so it is better to use stratified sampling in this scenario

```{r}
set.seed(2178)

n0=sum(framingham$TenYearCHD==0)
n1=sum(framingham$TenYearCHD==1)
s0=sample(which(framingham$TenYearCHD==0),0.7*n0)
s1=sample(which(framingham$TenYearCHD==1),0.7*n1)
s=c(s0,s1)
...
[^6]: Yet another metric we won't cover in these notes are *Brier Scores*, defined as the MSE between the predicted probabilities and the actual binary values. I put the name here so you know what it is if you see it in future performance analysis ventures.
...
Interestingly, this gave the same value as optimizing balanced accuracy^[This is no coincidence since 
$$
Balanced \,  Accuracy= \frac{TPR+(1-FPR)}{2} + 1/2 + (TPR-FPR)/2.
$$]

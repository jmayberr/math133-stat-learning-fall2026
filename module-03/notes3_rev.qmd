...
Recall that if we have a single predictor $X$, the basic model behind logistic regression is of the form $$p(X) = h(\beta_0 + \beta_1 X)$$ where $h$ is the logistic function. Similarly, with multiple predictors we have $$p(X) = h(\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p).$$ So while we can use the sign of the $\beta_i$ to determine if the probability of success goes up or down as we increase $X_i$, we cannot say anything more explicit from this formulation. First, we have to take a slightly different view of the logistic model and talk about odds.

If you take a walk through the garden of algebra[^1], you can rewrite the logistic equation as $$g(p(X)) =  \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p$$ where $g$ is given by the "link function[^2]" $$g(x) = \log(x/(1-x))$$ In other words, $$\log \left(\frac{p(X)}{1 - p(X)}\right) =  \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_p X_p$$ The lefthand side is called the "log-odds" function. When we look at a coefficient $\beta_i$, it is telling us the change in the log-odds of success when we increae predictor $X_i$ by 1. For example, in the framingham model, the log-odds of getting CHD are about 0.55 higher for males than females.

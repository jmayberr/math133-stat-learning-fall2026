...
Looking for random relationships between a response and a large number of predictors is called data mining (or the more sinister term "data manipulation") and it can lead to very misleading results. In fact, data mining has been the downfall of more than one researcher over the years[^1]. I have read plenty of papers myself that look at a bunch of random correlations, find some that are significant by chance, and move on, feeling good about themselves for discovering new false patterns in the universe.

[^1]: One of the most publicized was about the "food guy" Brian Wansink; [read more here](https://www.vox.com/science-and-health/2018/9/19/17879102/brian-wansink-cornell-food-brand-lab-retractions-jama)
...
This one is a little more complicated to code directly because you have to randomly split the data into K folds of roughly equal size[^2]. We use the `cut` function for this purpose. Here is a demonstration with $K=10$:

[^2]: They will be equal if $K$ divides $n$ but if not some folds may have an extra observation.
...
I'll leave it to you to adapt the cross-validation code in bonus programming practice below.

## Practice

The dataset [fantasy2025](https://raw.githubusercontent.com/jmayberr/math133-stat-learning-fall2026/refs/heads/main/Datasets/fantasy2025.csv) contains information on fantasy football points in a full points per reception league (`fantasy_points_ppr`) for all RBs, WRs, and TEs in the 2025 season by week. It also contains weighted 12 week averages of each players other game statistics (carries per game, touches per game, etc), age, and strength of defense.

Load this data into R and apply `drop_na()` to filter out missing values before completing 1-3 below.

1.  Compare the following three linear models for predicting `fantasy_points_ppr` based of overall $R^2$ values:
    - Model 1: fantasy_points_ppr\~position+ carries_pg+rushing_yards_pg+ targets_pg+receptions_pg+receiving_yards_pg+ rushing_tds_pg +receiving_tds_pg
    - Model 2: fantasy_points_ppr\~position+ carries_pg+rushing_yards_pg+ targets_pg+receptions_pg+receiving_yards_pg+ rushing_tds_pg +receiving_tds_pg+ age+def_fantasy_points_pg
    - Model 3: fantasy_points_ppr\~position\*(carries_pg+rushing_yards_pg+ targets_pg+receptions_pg+receiving_yards_pg+ rushing_tds_pg +receiving_tds_pg+ age+def_fantasy_points_pg)
2.  Now compare all three models from 1 using 10-fold CV (LOOCV takes took long in this scenario).
3.  Rank your models based on the answers to 1 and 2, noting any discrepancies.
...
*Bonus Bonus*: Modify the procedure to compute cross-validated balanced accuracy instead. Hint: Instead of accumulating SSE across folds, think about how to accumulate the confusion matrix components.

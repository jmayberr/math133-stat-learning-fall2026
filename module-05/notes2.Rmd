...
* Compute the mean ("center") of each cluster.
...
ggplot(toy_df,aes(x=x,y=y,color=as.factor(Cluster3Final)))+geom_point()+scale_color_manual(values=c("#56B4E9","#117A65","#E69F00"),name="Cluster")
...
electionPC |>
  ...
  geom_point(shape=1)+
  geom_text()+
  scale_color_manual(values=c("black","#e35205","#56B4e9","#009e73","#cc79a7"),name="Cluster")
```
...
This is the clustering equivalent of SSE. Why? $\bar{x}_j$ is kind of like the "predicted" value of a point $x_{ij}$ in cluster $j$ similar to $\hat{y}_j$ in typical regression models and the $x_{ij}$ are like the $y_j$ so we are looking at how close the values are to the centers. The equivalent of RMSE would be
...
electionPC |>
  ...
  geom_point(shape=1)+
  geom_text()+
  scale_color_manual(values = c("#009E73", "#0072B2", "#D55E00"),name="Cluster")
```
...
Each row is the center of a different cluster. Its helpful to visualize them with some kind of line, bar, or heat map. We'll show all three to give you choices (yay). The only complication being that we first have to convert the output of centers to a dataframe and use the pivot_longer function to bring together all of the center values into a new column called "value" with another column called "statistic" that stores the characteristic name:
...
Next, the line chart (Profile Plot)
```{r}
means |>
...
means |>
  ...
  scale_fill_distiller(
    palette = "RdBu",
    direction = -1   # red = low, blue = high; can flip with 1
  ) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
election2016 |>
  ...
  geom_point()+
  geom_text()+
  scale_color_manual(name="Cluster",values= c("#2166AC", "#7B3294", "#B2182B"))

```

Missouri looks like it should be in the blue cluster (for example), but remember, this is only one possible 2D snapshot of the nine-dimensional space. If we look at a different one, it looks more purple: 
...
However, its worth noting that all of the cluster assignments are the same, whether we use the full dataset or just the first two components!

```{r}
sum(electionPC$Cluster3==election2016$Cluster3)
```

So if clustering alone is your goal, performing PCA as a preliminary step can be valuable. 

## Using clustering for supervised prediction even though its unsupervised

Although clustering is an unsupervised method, we could use it as a classification scheme if we come across a variable that merits predicting. For example, suppose we compare clusters with the election outcome variable (red/blue) from the election2016 dataset. 

```{r}
table(election2016$Cluster3,election2016$outcome)
```

If a state is in clusters 1 or 2, we would do well to predict Red while cluster 3 goes better with Blue. The error rate for this scheme would be 

```{r}
wrong_red=sum(election2016$Cluster3 %in% c(1,2)&election2016$outcome=="Blue")
wrong_blue=sum(election2016$Cluster3==3&election2016$outcome=="Red")
(wrong_red+wrong_blue)/50
```
Not terrible, but not as good as other classification schemes (KNN or logistic)
...
Before closing out these notes though, I would like to mention one additional issue with K-means: its sensitivity to outliers. If you have three nice clusters and we add a new point that is completely separate from all three, then one of the cluster centers may shift drastically to accommodate the new point. The moral is that relying on means always makes you sensitive to outliers^[Stats joke: Two statisticians are having a drink at a bar when Elon Musk walks in. One nudges the other and says, "On average, everyone here just got way richer!" They laugh—until they remember they lost their jobs because of DOGE and fall into despair.]. 

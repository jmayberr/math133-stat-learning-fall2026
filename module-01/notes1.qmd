...
In fact, we could compute the mean of Y for each value of X as follows[^1] to estimate $m,b$:

[^1]: We'll learn a quicker way of getting the mean for different groups next class.
...
The dataset election2016.csv in the shared folder contains the results of the 2016 presidential election by state along with key demographic information (Source: [FiveThirtyEight](https://github.com/fivethirtyeight/data/tree/master/hate-crimes)). You can load it into Rstudio and look at the first few rows using the following commands which pull the dataset from the course github repository:
...
The tidyverse package contains ggplot2, the premier plotting package[^2] in R and dare I say any data science software? The following creates a scatterplot of *trump.vote* vs *non.white*

[^2]: Details on how to use this package are covered in the Module 1 R Tools Supplement.
...
f(X)= \beta_0 + \beta_1 X
$$ is a linear function[^3] and so estimating it is equivalent to estimating the two parameters $\beta_0, \beta_1$ with some $\hat{\beta}_0, \hat{\beta}_1$. The predicted outcomes would then be $$
...
[^3]: We are getting away from using $mx+b$ because as we add more variables, we need an easy way to index coefficients.
...
$$\
Minimize means choosing estimates $\hat{\beta}_0,\hat{\beta}_1$ to make this quantity as small as possible. This becomes a calculus or linear algebra problem[^5] and solving it gives the least squares estimates of $\beta_0, \beta_1$. The R function lm will compute them by

[^4]: Sometimes called a cost function

[^5]: See the Module 1 Theory Supplement for further details.
